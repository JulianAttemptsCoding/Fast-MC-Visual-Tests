...
```powershell
npm install
python scripts/export_public_data.py --source "..\Fast MC CBSC\dashboard\public\data" --destination "public\data"
npm run test
npm run dev
```
...
After a QA-passing Vertex epoch has been synced into the source dashboard, run
the export command above. The exporter verifies hashes, geometry, the fixed
selection, the 50-by-5 contract, and zero test events before writing
deterministic gzip artifacts. Commit and push; GitHub Pages deploys the update.
